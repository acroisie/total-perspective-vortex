...
class CustomCSP(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        if np.any(eigenvals_comp <= 0):
            print(f"Warning: Found {np.sum(eigenvals_comp <= 0)} non-positive eigenvalues")
    ...
    def transform(self, X):
        ...
        if n_channels != self.filters_.shape[0]:
            raise ValueError(f"Number of channels mismatch: expected {self.filters_.shape[0]}, got {n_channels}")
        ...
        for trial in range(n_trials):
            for comp in range(self.n_components):
                ...
                if self.log:
                    ...
                    features[trial, comp] = np.log(variance + 1e-10)  # Add small constant to avoid log(0)
                else:
...
def test_custom_csp():
    ...
    y = np.concatenate([np.zeros(n_trials_per_class), np.ones(n_trials_per_class)])
